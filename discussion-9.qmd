...
```{r}
# tester chunk bc next chunk is getting funky
library(forecast)
library(tidyverse)
...
components(dcmp) %>% 
  ...
  autoplot(avg_temp, color = 'grey') +  # ogdata
  geom_line(aes(y = season_adjust), color = 'lightblue') +
...
-   blueline is the custom fit algorithim \## Use this to evaluate current data to determine what to extrapolate Default is what typically fits the best, adjusting parameters is very subjective
...
hist(hw_future$residuals, breaks = 20)
```
